    ...
    <div className="space-y-8 pb-20 md:pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-600 font-medium">Manage quest catalogs, inspect AI verdicts & EcoGuard verification queue.</p>
        </div>
      ...
      {activeTab === 'review' && (
        <div className="space-y-4">
          <h2 className="text-lg font-extrabold text-gray-900">Pending & Flagged Verification Submissions</h2>

          <GlassCard className="overflow-x-auto p-0">
            <table className="w-full text-left text-xs text-gray-900">
              <thead className="border-b border-gray-100 bg-gray-50 font-mono uppercase text-gray-500 font-bold">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 font-semibold">
                {proofSubmissions.map((proof: ProofSubmission) => (
                  <tr key={proof.id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="py-3 px-4 uppercase font-mono text-emerald-700 font-bold">{proof.verificationType.replace('_', ' ')}</td>
                    <td className="py-3 px-4 font-mono">
                      <span className={proof.aiVerdict === 'Approved' ? 'text-emerald-700 font-extrabold' : 'text-amber-700 font-extrabold'}>
                      ...
                      {proof.status === 'approved' ? (
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="rounded-lg bg-emerald-100 px-2.5 py-1 font-mono text-[11px] font-bold text-emerald-800 hover:bg-emerald-200"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => handleAuditReview(proof.id, false)}
                            className="rounded-lg bg-rose-100 px-2.5 py-1 font-mono text-[11px] font-bold text-rose-800 hover:bg-rose-200"
                          >
                            Reject
                          </button>
                        </div>
      ...
      {activeTab === 'quests' && (
        <div className="space-y-6">
          <GlassCard className="space-y-4">
            <h2 className="text-lg font-extrabold text-gray-900">Author New Quest Challenge</h2>
            ...
            <form onSubmit={handleCreateQuest} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-mono font-bold text-gray-700 mb-1">Quest Title</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div>
                <label className="block text-xs font-mono font-bold text-gray-700 mb-1">Category</label>
                <select
                  value={newCategory}
              ...
              <div>
                <label className="block text-xs font-mono font-bold text-gray-700 mb-1">EcoXP Reward</label>
                <input
                  type="number"
                  value={newXp}
              ...
              <div>
                <label className="block text-xs font-mono font-bold text-gray-700 mb-1">Verification Level</label>
                <select
              ...
              <div className="col-span-2">
                <label className="block text-xs font-mono font-bold text-gray-700 mb-1">Description</label>
                <textarea
                  value={newDesc}
              ...
              <div className="col-span-2">
                <button
                  type="submit"
                  className="rounded-xl bg-[#111827] px-6 py-2.5 text-xs font-bold text-white hover:bg-black transition-all shadow-sm"
                >
